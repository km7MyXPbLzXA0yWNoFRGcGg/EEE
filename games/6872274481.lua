...
run(function()
	...
	-- Original AttackRemote placeholder — replaced/wrapped below when real instance available
	local AttackRemote = {FireServer = function() end}
	task.spawn(function()
		AttackRemote = bedwars.Client:Get(remotes.AttackEntity).instance
	end)

	-- ======= Rate-limiting wrapper for AttackRemote =======
	-- Config
	local RATE_LIMIT = 300      -- max sends per window
	local RATE_WINDOW = 60      -- seconds window and cooldown duration

	-- Sliding-window state
	local _sendCount = 0
	local _windowStart = tick()
	local _rateLimited = false
	local _rateLimitedUntil = 0

	-- Mark client rate-limited for RATE_WINDOW seconds
	local function markRateLimited(source)
		_rateLimited = true
		_rateLimitedUntil = tick() + RATE_WINDOW
		pcall(function()
			-- Change to GUI notification if desired
			warn(("Rate limited by %s — blocking remotes for %ds"):format(tostring(source or "client"), RATE_WINDOW))
		end)
	end

	-- Check if allowed to send now
	local function canSend()
		-- If currently flagged rate-limited, check expiry
		if _rateLimited then
			if tick() >= _rateLimitedUntil then
				_rateLimited = false
				_sendCount = 0
				_windowStart = tick()
				return true
		...
		if tick() - _windowStart >= RATE_WINDOW then
			_windowStart = tick()
			_sendCount = 0
		end

		-- If we've reached the limit, mark rate-limited and block
		if _sendCount >= RATE_LIMIT then
			markRateLimited("local-throttle")
			return false
		end
	...
	-- Create a safe wrapper around a real remote instance
	local function installSafeAttackRemote(realRemote)
		if not realRemote then return nil end
		-- If already wrapped, return as-is
		if realRemote._isSafeWrapper then return realRemote end

		local wrapper = {}
		wrapper._real = realRemote
		wrapper._isSafeWrapper = true

		-- Preserve FireServer API but enforce rate-limiting
		function wrapper:FireServer(...)
			-- Block while rate-limited
			if not canSend() then
				-- drop the send (per requested behavior)
				return
			end
			...
			-- Call the real remote safely
			local ok, err = pcall(function()
				-- Some remotes might have different callable shapes; call FireServer if present
				if type(self._real.FireServer) == "function" then
					self._real:FireServer(...)
				else
					-- Fallback try
					self._real:FireServer(...)
				end
			...
			-- Heuristic: if an error occurs and mentions 'rate', mark rate-limited
			if not ok and tostring(err):lower():find("rate") then
				markRateLimited("fire-error")
			end
		end

		-- Mirror other fields/methods via metatable so code inspecting AttackRemote still works
	...
	-- Background task: wait for AttackRemote to resolve, then wrap; also attempt heuristic detection
	task.spawn(function()
		-- Wait briefly for the real AttackRemote to become available
		local start = tick()
		while (not AttackRemote or not AttackRemote.FireServer) and tick() - start < 5 do
			task.wait(0.05)
		end

		-- If we have a real remote instance, wrap it
		if AttackRemote and AttackRemote.FireServer then
			local safe = installSafeAttackRemote(AttackRemote)
			if safe then
				AttackRemote = safe
			end
		end

		-- Heuristic server-metric detection:
		-- Scan 'remotes' for names with "metric"/"rate" and hook OnClientEvent to detect a rate-limit message.
		pcall(function()
			if type(remotes) == "table" then
				for name, rem in pairs(remotes) do
					local lname = tostring(name):lower()
					if lname:find("metric") or lname:find("metrics") or lname:find("rate") then
						local ok, inst = pcall(function() return bedwars.Client:Get(rem).instance end)
						if ok and inst and inst.OnClientEvent then
							pcall(function()
								inst.OnClientEvent:Connect(function(...)
									local args = {...}
									for _, v in ipairs(args) do
										local s = tostring(v):lower()
										if s:find("ratelimit") or s:find("rate limit") or s:find("too many") then
											markRateLimited(("server-metric (%s)"):format(name))
											break
										end
									end
								end)
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if actualRoot then
									...
									-- AttackRemote:FireServer is now rate-limited by the wrapper above.
									AttackRemote:FireServer({
										weapon = sword.tool,
										chargedAttack = {chargeRatio = 0},
										entityInstance = v.Character,
